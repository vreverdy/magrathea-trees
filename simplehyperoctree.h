...
class SimpleHyperOctree final
{
    ...
    public: 
        ...
        explicit inline SimpleHyperOctree(const unsigned int ilvl, const unsigned int nref = 0, const Data& data = Data());
    //@}
...
template <typename Type, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container>
inline SimpleHyperOctree<Type, Index, Data, Dimension, Position, Extent, Element, Container>::SimpleHyperOctree(const unsigned int ilvl, const unsigned int nref, const Data& data)
: _container(Container())
{
    static const Element element = Element(Index(), data);
    Index idx = Index();
    idx = idx.previous(ilvl, nref);
    _container.resize(Index::indices(ilvl, nref), element);
