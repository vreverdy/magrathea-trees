...
{
    static constexpr auto comb_result = comb<Type>(Dimension+1, Dimension) << (Bits%(Dimension+1));
    //constexpr auto dimp1 = std::integral_constant<Type, Dimension+1>::value;
    //constexpr auto zero = std::integral_constant<Type, 0>::value;
    //std::cout<<comb_result<<std::endl;
    //return ((Bits-tzcnt<Type, comb_result>(_data))/dimp1)+(_data != zero);
    return __builtin_popcountll(_data & comb_result);
}
